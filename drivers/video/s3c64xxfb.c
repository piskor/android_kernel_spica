...
static struct s3c_fb_drvdata s3c_fimd = {
...
static void s3cfb_set_fimd_info(struct s3c64xxfb_platform_data *pdata)
{
	if (!pdata->dither) {
		s3c_fimd.dithmode = 0;
	} else {
...
static void s3cfb_activate_var(struct s3c_fb_info *fbi, struct fb_var_screeninfo *var)
{
	...
	fbi->fb.fix.line_length = var->xres_virtual * fbi->bytes_per_pixel;
...
static int s3cfb_pan_display(struct fb_var_screeninfo *var, struct fb_info *info)
{
	...
	fbi->fb_start_addr = fbi->map_dma + var->yoffset * fbi->fb.fix.line_length;
	fbi->fb_end_addr = fbi->fb_start_addr + var->yres * fbi->fb.fix.line_length;
...
static void s3cfb_setup_window(struct s3c_fb_info *fbi)
{
	...
	unsigned long size, offset, line_size;
	...
	fbi->wincon	= S3C_WINCONx_ENWIN_F_DISABLE;
	fbi->vidosda	= S3C_VIDOSDxA_OSD_LTX_F(0)
			| S3C_VIDOSDxA_OSD_LTY_F(0);
	fbi->vidosdb	= S3C_VIDOSDxB_OSD_RBX_F(s3c_fimd.xres - 1)
			| S3C_VIDOSDxB_OSD_RBY_F(s3c_fimd.yres - 1);
	fbi->vidosdc	= S3C_VIDOSDxC_ALPHA1_B(S3C_FB_MAX_ALPHA_LEVEL)
			| S3C_VIDOSDxC_ALPHA1_G(S3C_FB_MAX_ALPHA_LEVEL)
			| S3C_VIDOSDxC_ALPHA1_R(S3C_FB_MAX_ALPHA_LEVEL);
	fbi->vidosdd	= S3C_VIDOSDxD_OSDSIZE(var->xres * var->yres);

	fbi->fb_start_addr = fbi->map_dma;
	fbi->fb_end_addr = fbi->map_dma + size;

#if 0 /* Not implemented yet */
	/* Display boot logo if needed */
	if (!win_num && s3c_fimd.logo_base)
	...
	/* Activate default window video mode */
	s3cfb_set_par(&fbi->fb);
}
					...
					const char *drv_name, int index)
{
	...
	finfo->fb.var.bits_per_pixel = 8;
#elif defined(CONFIG_FB_S3C64XX_BPP_16)
	finfo->fb.var.bits_per_pixel = 16;
#elif defined(CONFIG_FB_S3C64XX_BPP_24)
	finfo->fb.var.bits_per_pixel = 32;
#else
#error Wrong BPP mode.
#endif
	...
	/* Allocate memory as for 32 BPP mode for dynamic mode changes */
...
static int __init s3cfb_probe(struct platform_device *pdev)
{
	...
	struct s3c64xxfb_platform_data *pdata = pdev->dev.platform_data;
